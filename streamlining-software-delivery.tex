...
Achieving quality is above all an ambition. No process or practise will account for quality if the developers are not willing to pursue it. A team must set mutual working principles that define how development will aim to deliver quality. These include objectives from coding conventions to reviewing each others work. Quality should be a concerted effort.
...
Indeed, the leading edge of Continuous Experimentation is even starting to favour experiments over defined test suites. Instead of rigorously testing features beforehand, automatic analytics are run in production. Heuristics are used to immediately discovers issues and alert about their consequences.
